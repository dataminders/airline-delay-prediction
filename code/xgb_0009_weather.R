...
# Need to add weather.
# LB 0.69523
train <- fread('../Data/train_weather_prepared.csv')
response <- train[['ARR_DEL15']]
test <- fread('../Data/test_weather_prepared.csv')[, ARR_DEL15 := NULL][, ARR_DEL15 := NA]
id <- test[['id']]
...
write.csv(predict(clf.valid, dval), "../cv/xgb_0009_weather.csv", quote = FALSE, row.names = FALSE)
...
write.csv(subs, "../lb/xgboost_0009_weather.csv", quote = FALSE, row.names = FALSE)