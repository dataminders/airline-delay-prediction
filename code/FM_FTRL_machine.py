...
class FM_FTRL_machine(object):
    ...
    def predict_raw(self, x):
        ...
        alpha_fm, beta_fm = self.alpha_fm, self.beta_fm
        L1_fm, L2_fm      = self.L1_fm, self.L2_fm
        
        # first order weights model
        n, z, w = self.n, self.z, self.w
        
        # FM interaction model
        n_fm, z_fm, w_fm = self.n_fm, self.z_fm, self.w_fm
        raw_y = 0.
...
def data(filePath, hashSize):
    for t, line in enumerate(DictReader(open(filePath))):
        y = float(line['ARR_DEL15'])
        del line['ARR_DEL15']
        x = []
        
        for key in line:
            value = line[key]
            index = abs(hash(key+'_'+str(value))) % hashSize + 1  # zero index for bias term
            x.append(index)
